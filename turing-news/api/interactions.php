...
// SHARE - Aggiungi condivisione
if ($method === 'POST' && isset($data['action']) && $data['action'] === 'share') {
    $userId = validateToken();
    $postId = (int)($data['post_id'] ?? 0);
    ...
    if ($stmt->execute([$postId, $userId])) {
        // Conta le condivisioni totali
        $stmt = $db->prepare("SELECT COUNT(*) as count FROM shares WHERE post_id = ?");
        $stmt->execute([$postId]);
        $sharesCount = $stmt->fetch()['count'];
        
        jsonResponse([
            'success' => true,
            'message' => 'Post condiviso',
            'data' => [
                'shares_count' => (int)$sharesCount
            ]
        ], 201);
    } else {
        jsonResponse(['success' => false, 'message' => 'Errore durante la condivisione'], 500);
    }
}

// LIKED POSTS - Post apprezzati dall'utente
...
// CHECK LIKE - Verifica se l'utente ha messo like
