...
async function renderPosts(posts) {
    ...
    for (const post of posts) {
        ...
        if (user && user.id === post.user_id) {
            deleteButtonHtml = `<button class="delete-btn" data-post-id="${post.id}">Elimina</button>`;
        }
...
function setupPostInteractions(postId) {
    ...
    const deleteBtn = document.querySelector(`.delete-btn[data-post-id="${postId}"]`);
    if (deleteBtn) {
        deleteBtn.addEventListener('click', async function() {
            ...
            try {
                ...
                const postElement = document.querySelector(`.news-card[data-post-id="${postId}"]`);
                if (postElement) {
                    postElement.remove();
                }
                
                // Usa la funzione di notifica esistente
